...
export function PracticeCard({ practice }: PracticeCardProps) {
  return (
    <Link href={practice.link} legacyBehavior>
      <a target={practice.link.startsWith('http') ? '_blank' : '_self'} rel={practice.link.startsWith('http') ? 'noopener noreferrer' : ''} className="block group">
