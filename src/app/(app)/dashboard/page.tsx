...
export default function DashboardPage() {
      ...
      <div className="container mx-auto">
        <div className="mb-6 md:mb-8">
            <div className="grid gap-1">
              <Skeleton className="h-9 w-1/2 md:w-1/3" /> {/* PageHeader title skeleton */}
              <Skeleton className="h-5 w-3/4 md:w-1/2" /> {/* PageHeader description skeleton */}
            </div>
        </div>
  ...
  return (
    <div className="container mx-auto">
      {/* PageHeader title (H1) and description (p) will use global typography styles */}
      <PageHeader
          ...
          <Card key={feature.href} className="h-full flex flex-col hover:border-primary transition-colors hover:shadow-lg rounded-lg">
            <Link href={feature.href} className="flex flex-col flex-grow">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <CardTitle className="font-headline">{feature.label}</CardTitle> {/* Explicitly font-headline */}
                <feature.icon className="h-6 w-6 text-muted-foreground" />
              </CardHeader>
              <CardContent className="flex-grow">
                {/* CardDescription is text-sm by default from ui/card.tsx */}
                <CardDescription>
                  Access {feature.label.toLowerCase()} tools and insights.
                </CardDescription>
              </CardContent>
              <CardContent className="pt-0">
                  <div className="text-sm font-medium text-primary flex items-center"> {/* Small text, font-medium */}
                  Go to {feature.label}
